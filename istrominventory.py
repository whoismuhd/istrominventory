                ...
                for category_name, subcategories in categories.items():
                    ...
                    for subcategory_key, category_items in subcategories.items():
                        ...
                        if not actual_df.empty:
                            ...
                            # Display empty dataframe if no data
                            empty_df = pd.DataFrame(columns=['S/N', 'Item', 'Qty', 'Unit', 'Unit Cost', 'Total Cost'])
...
with tab4:
    ...
    if is_admin():
        ...
        # Use regular widgets (not form) so rejection reason field can appear/disappear dynamically
        ...
        # Submit button
        if st.button("Apply", type="primary", key="approve_reject_submit"):
