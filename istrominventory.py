...
if 'dark_mode' not in st.session_state:
    ...
    (function() {
        ...
        const savedPref = localStorage.getItem('dark_mode');
        
        // If no saved preference, detect system preference
        if (savedPref === null) {
            const systemPrefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
            localStorage.setItem('dark_mode', systemPrefersDark ? 'true' : 'false');
...
{current_project}
    ...
    # Initialize dark mode in session state from localStorage or system preference
    if 'dark_mode' not in st.session_state:
        # JavaScript will set localStorage based on system preference if not set
        # We'll check it via a hidden component that reads localStorage
        st.session_state.dark_mode = False
        
        # Use JavaScript to sync localStorage value to session state
        st.markdown("""
        <script>
        (function() {
            const darkModePref = localStorage.getItem('dark_mode');
            if (darkModePref === 'true') {
                // Signal to Python that dark mode should be enabled
                // We'll use a query parameter approach
                const url = new URL(window.location);
                if (!url.searchParams.has('dark_mode_init')) {
                    url.searchParams.set('dark_mode_init', 'true');
        ...
        # Check if we're initializing from localStorage
        if st.query_params.get('dark_mode_init') == 'true':
            st.session_state.dark_mode = True
            # Remove the parameter to avoid loops
            st.query_params.pop('dark_mode_init', None)
    ...
    with col1:
        dark_mode = st.toggle(
            ...
            help="Switch between light and dark theme. Set to 'Use System' to match your device settings.",
            key="dark_mode_toggle"
        )
    with col2:
        if st.button("🌐 Use System", help="Match your device theme settings", key="use_system_theme"):
            # Detect system preference via JavaScript
            st.markdown("""
            <script>
            const systemPrefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
            localStorage.setItem('dark_mode', systemPrefersDark ? 'true' : 'false');
    ...
    # Apply dark mode CSS when enabled - COMPREHENSIVE VERSION
    if st.session_state.dark_mode:
        st.markdown("""
        <style>
        /* Dark Mode Styles - Comprehensive with high contrast */
        :root {
            --dark-bg: #0f172a;
        ...
        /* Main content area - ensure all text is visible */
        ...
        * {
        ...
        /* Headers and text - high contrast */
        h1, h2, h3, h4, h5, h6,
        .stMarkdown,
        .stMarkdown *,
        p, div, span, li, td, th,
        ...
        /* Metrics - ensure all text is visible */
        ...
        /* Dashboard header cards - fix hardcoded colors */
